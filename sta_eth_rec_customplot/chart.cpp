...
void Chart::handleValueChanged(fft_size_t *arr)
{
    ...
    {
        // if ((float(arr[i])/4194304)-107.5 > -50)
            // qDebug()  << "Frequency: " << (*x)[i] << "Value: " << (float(arr[i])/4194304)-107.5;
            // qDebug()  << "Frequency: " << (x[i]) << "Value: " << (float(arr[i]));
        // if ((float(arr[i])/4194304)-107.5 > 0)
        //     (*y)[i] = -130;
        // else
            (*y)[i] = (float(arr[i])/4194304)-150-20;
            
            if((*y)[i]>0)
            printf("value max is:%f\tarray value:%d\tindex:%d\n",(*y)[i],arr[i],i);
    }
